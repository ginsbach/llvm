...
public:
  ...
  SyntaxTree(SyntaxTree const &) = default;
  SyntaxTree &operator=(SyntaxTree e) {
    std::swap(leaf_value, e.leaf_value);
    std::swap(node_value, e.node_value);
    return *this;
  }
  ...
  bool isleaf(const string &s) const {
    return node_value.empty() && leaf_value == s;
  }
  bool isnode() const { return !node_value.empty(); }
  bool isnode(const string &s) const {
    return !node_value.empty() && get_type() == s;
  }
  const string &get_leaf() const { return leaf_value; }
  const string &get_type() const { return node_value[0].get_leaf(); }
  size_t size() const {
    return node_value.empty() ? 0 : (node_value.size() - 1);
  }
...
vector<SyntaxTree> Parse(vector<pair<string, SyntaxTree>> bnf_spec,
                         string input);
...
ostream &operator<<(ostream &ostr, const vector<SyntaxTree> &x) {
  for (size_t i = 0; i < x.size(); i++) {
    if (!x[i].node_value.empty())
      ostr << (i ? " " : "(") << x[i].node_value << ",";
      ...
      ostr << (i ? " " : "(") << x[i].leaf_value << ",";
  }
  return ostr << (x.size() ? ")" : "()");
}

// This function parses the Backus-Naur form language specification. Different
// to the actual parser that the resulting grammar is informing, this is a
// hardcoded deterministic LALR(2) parser. The input has to be fairly well
// structured. In particular, all tokens have to be separated by whitespace, as
// the lexer is very primitive.
vector<pair<string, SyntaxTree>> ParseBNF(string input) {
  vector<string> tokens{""};
  for (auto c : input + " ) )") {
    if (isspace(c) && !tokens.back().empty())
      tokens.push_back("");
    if (!isspace(c))
  ...
  for (const auto &t : tokens) {
    if (t == "::=" || t == "|" || t == "[" || t == "]" || t == "{" ||
      ...
      stack.push_back(SyntaxTree(
          "reference", {SyntaxTree(string(t.begin() + 1, t.end() - 1))}));
    else if (t.front() == '\'' && t.back() == '\'' && t.size() >= 2)
      stack.push_back(SyntaxTree(
          "literal", {SyntaxTree(string(t.begin() + 1, t.end() - 1))}));
    else
      stack.push_back(SyntaxTree("literal", {SyntaxTree(t)}));

    while (true) {
      auto len = [&s = stack](size_t n) -> bool { return s.size() >= n; };
      auto get = [&s = stack](size_t n) -> const SyntaxTree & {
        return s.rbegin()[n];
      };
      auto mov = [&s = stack](size_t n) -> SyntaxTree {
        return move(s.rbegin()[n]);
      };
      auto set = [&s = stack](size_t n, SyntaxTree t) {
        s.rbegin()[n] = t;
        s.erase(s.end() - n, s.end() - 2);
      };
      ...
      if (len(4) && get(3).isnode() && get(2).isnode() &&
          !get(1).isleaf("::=")) {
        set(3, get(3).isnode("sequence")
                   ? add(3, 2)
                   : SyntaxTree("sequence", {mov(3), mov(2)}));
      } else if (len(5) && get(4).isleaf("[") && get(3).isnode() &&
                 get(2).isleaf("]")) {
        set(4, SyntaxTree("optional", get(3).isnode("sequence")
                                          ? mov(3).children()
                                          : vector<SyntaxTree>{mov(3)}));
      } else if (len(5) && get(4).isleaf("{") && get(3).isnode() &&
                 get(2).isleaf("}")) {
        set(4, SyntaxTree("repeat", get(3).isnode("sequence")
                                        ? mov(3).children()
                                        : vector<SyntaxTree>{mov(3)}));
      } else if (len(5) && get(4).isnode() && get(3).isleaf("|") &&
                 get(2).isnode() &&
                 (get(0).isleaf("::=") || get(1).isleaf("|") ||
                  get(1).isleaf(")") || get(1).isleaf("]") ||
                  get(1).isleaf("}"))) {
        set(4, get(4).isnode("choice")
                   ? add(4, 2)
                   : SyntaxTree("choice", {mov(4), mov(2)}));
      } else if (len(5) && get(4).isnode("literal") && get(3).isleaf("::=") &&
                 get(2).isnode() &&
                 ((get(1).isleaf(")") && get(0).isleaf(")")) ||
                  (get(1).isnode("literal") && get(0).isleaf("::=")))) {
  ...
  if (stack.size() > 2) {
    stringstream sstr;
    sstr << "Failed to parse BNF file, as not all syntax collapsed to "
  ...
  for (size_t i = 0; i < node_value.size();) {
    ...
    if (child.isnode() && child.get_type().find('@') != string::npos) {
      if (child.node_value.size() > 1) {
        ...
        node_value.insert(node_value.begin() + i + 1,
                          make_move_iterator(child.node_value.begin() + 2),
                          make_move_iterator(child.node_value.end()));
      } else
        node_value.erase(node_value.begin() + i);
      i += child.size();
    } else {
...
public:
  ParseBranch(SyntaxTree h, shared_ptr<ParseBranch> t)
      : SyntaxTree(h), tail(t) {}
...
struct ParserRules {
  ...
  function<void(const ParseBranch &, vector<shared_ptr<ParseBranch>> &)>
      keep_rule;
};
...
vector<SyntaxTree> Parse(vector<pair<string, SyntaxTree>> bnf_spec,
  ...
  for (auto c : input) {
    ...
    if (isspace(c) && !tokens.back().empty())
      ...
      if (c == '.' && tokens.size() > 2 && tokens.back().empty() &&
          tokens.rbegin()[1] == ".") {
        tokens.rbegin()[1] = "..";
        continue;
      }
...
bool ParseBranch::operator==(const ParseBranch &o) const {
  if ((const SyntaxTree &)*this != (const SyntaxTree &)o)
    return false;
  if (tail == o.tail)
    return true;
  if (tail == nullptr || o.tail == nullptr)
    return false;
  return *tail == *o.tail;
}
...
bool SyntaxTree::operator!=(const SyntaxTree &t) const {
  if (isleaf())
    return !t.isleaf(get_leaf());
  if (!isnode())
    return t.isleaf() || t.isnode();
  if (!t.isnode(get_type()) || size() != t.size())
    return true;
...
ParseBranch::operator vector<SyntaxTree>() && {
  ...
  for (size_t i = 0; i < trees.size(); i++) {
    if (trees[i].isnode("#"))
      ...
      throw string("There was a syntax error.");
  }

  return trees;
}

// This is a workaround to allow building without enabling C++20 in the
// compiler.
template <class SType, class EType>
bool contains(const SType &S, const EType &E) {
  ...
  while (!stack.empty()) {
    ...
    auto str_to_refsyntax = [](unordered_set<string> vec) {
      ...
      for (const auto &x : vec)
        result.push_back(SyntaxTree("reference", {SyntaxTree(x)}));
      return result;
    };
    auto extract_refsyntax = [](const vector<SyntaxTree> &vec) {
      unordered_set<string> result;
      for (const auto &x : vec)
        if (x.isnode("reference"))
          result.insert(x[0].get_leaf());
      return result;
    };
    ...
    auto sequence_cont = [str_to_refsyntax](unordered_set<string> prev,
                                            vector<SyntaxTree> s) {
    ...
    if (r.isnode("literal"))
      lit_rule.emplace_back(l, make_set{}, r[0].get_leaf());
    else if (r.isnode("reference"))
      ref_rule.emplace_back(l, make_set{}, make_set{r[0].get_leaf()});
    else if (r.isnode("string"))
      str_rule.emplace_back(l, make_set{});
    else if (r.isnode("number"))
      nbr_rule.emplace_back(l, make_set{});
    else if (r.isnode("choice")) {
      auto children = move(r).children();

      auto refs = extract_refsyntax(children);
      if (!refs.empty())
        ref_rule.emplace_back(l, make_set{}, refs);
      ...
      for (auto &part : move(r).children()) {
        ...
        if (part.isnode("choice") && old_tmps.empty()) {
          ...
          stack.push_back(
              {name, sequence_cont(old_tmps, move(part).children())});
          ...
          auto find_it =
              find_if(lit_rule.begin(), lit_rule.end(), [&](const auto &x) {
                return (get<1>(x) == old_tmps &&
                        get<2>(x) == part[0].get_leaf() &&
                        get<0>(x).find('@') != string::npos);
              });
          if (find_it != lit_rule.end())
            name = get<0>(*find_it);
          else
            lit_rule.emplace_back(name, old_tmps, part[0].get_leaf());
        } else if (part.isnode("reference")) {
          auto find_it =
              find_if(ref_rule.begin(), ref_rule.end(), [&](const auto &x) {
                return (get<1>(x) == old_tmps &&
                        get<2>(x) == make_set{part[0].get_leaf()} &&
                        get<0>(x).find('@') != string::npos);
              });
          if (find_it != ref_rule.end())
            name = get<0>(*find_it);
          else if (old_tmps.empty())
            name = part[0].get_leaf();
          else
            ref_rule.emplace_back(name, old_tmps, make_set{part[0].get_leaf()});
        } else if (part.isnode("string"))
          str_rule.emplace_back(name, old_tmps);
        else if (part.isnode("number"))
          nbr_rule.emplace_back(name, old_tmps);
        else
          throw string("Failed to process BNF file, some sequence rule cannot "
                       "be dissected fully.");
      ...
      throw string("Syntax Error in BNF file: No decomposition to break down "
                   "complex rule.");
  }

  unordered_set<string> types_with_ref_cont{"#"};
  unordered_set<string> types_with_any_cont{"#"};
  for (const auto &rule : lit_rule)
    types_with_any_cont.insert(get<1>(rule).begin(), get<1>(rule).end());
  for (const auto &rule : ref_rule)
    types_with_ref_cont.insert(get<1>(rule).begin(), get<1>(rule).end());
  for (const auto &rule : ref_rule)
    types_with_any_cont.insert(get<1>(rule).begin(), get<1>(rule).end());
  for (const auto &rule : str_rule)
    types_with_any_cont.insert(get<1>(rule).begin(), get<1>(rule).end());
  for (const auto &rule : nbr_rule)
    types_with_any_cont.insert(get<1>(rule).begin(), get<1>(rule).end());

  return {
      [=](const ParseBranch &b, vector<ParseBranch> &out) {
        for (auto &rule : lit_rule)
          if ((get<1>(rule).empty() || b.tailtest(get<1>(rule))) &&
              b.isleaf(get<2>(rule)))
            out.emplace_back(
                b.fork(!get<1>(rule).empty(), false, get<0>(rule)));
        for (auto &rule : ref_rule)
          if ((get<1>(rule).empty() || b.tailtest(get<1>(rule))) &&
              b.isnode() && contains(get<2>(rule), b.get_type()))
            out.emplace_back(b.fork(!get<1>(rule).empty(), true, get<0>(rule)));
        for (auto &rule : str_rule)
          if ((get<1>(rule).empty() || b.tailtest(get<1>(rule))) &&
              b.isleaf() && !isdigit(b.get_leaf()[0]))
            out.emplace_back(b.fork(!get<1>(rule).empty(), true, get<0>(rule)));
      ...
      [=](const ParseBranch &b, vector<shared_ptr<ParseBranch>> &out) {
        if (b.tailtest(types_with_ref_cont, true) && b.isnode() &&
            contains(types_with_any_cont, b.get_type()))
          out.push_back(shared_ptr<ParseBranch>(new ParseBranch(move(b))));
      }};
}

// This function pattern matches on the fail of the branch.
bool ParseBranch::tailtest(const unordered_set<string> &p,
                           bool allow_null) const {
  return (allow_null && tail == nullptr) ||
         (tail && tail->isnode() && contains(p, tail->get_type()));
}
...
ParseBranch ParseBranch::fork(bool from_tail, bool keep_head, string n) const {
  if (from_tail && tail && keep_head)
    return ParseBranch(SyntaxTree(n, {*tail, *this}), tail->tail);
  else if (from_tail && tail)
    return ParseBranch(SyntaxTree(n, {*tail}), tail->tail);
  else if (keep_head)
    return ParseBranch(SyntaxTree(n, {*this}), tail);
  else
    return ParseBranch(SyntaxTree(n, {}), tail);
}
...
SyntaxTree SimplifyIDLConstraint(
    SyntaxTree in, const unordered_map<string, SyntaxTree> &lookup,
...
vector<SyntaxTree> ProcessIDLSpecs(const vector<SyntaxTree> &in_vector) {
  ...
  for (size_t i = 0; i + 1 < in_vector.size(); i++)
    if (in_vector[i + 1].isnode("export") &&
        in_vector[i].isnode("specification")) {
...
vector<SyntaxTree> NormaliseIDLVar(SyntaxTree in,
                                   const unordered_map<string, int> &indices,
...
SyntaxTree SimplifyIDLConstraint(
  ...
  if (in.isnode("atom") && in.size() == 1) {
    ...
    auto end =
        in.isnode("for") ? (begin + 1) : CollapseIDLIndex(in[3], indices);

    string type =
        string(in.get_type().begin(), in.get_type().begin() + 3) + "junction";

    vector<SyntaxTree> elements;
    for (int index = begin; index < end; index++) {
      indices[in[1].get_leaf()] = index;
      elements.push_back(
          SimplifyIDLConstraint(in[0], lookup, indices, collidx, renaming));
    }
    ...
    for (auto &child : move(in).children())
      elements.push_back(
          SimplifyIDLConstraint(child, lookup, indices, collidx, renaming));
    ...
    auto comp = [](const SyntaxTree &a, const SyntaxTree &b) -> bool {
      ...
      while (true) {
        ...
        if (it1->isnode("slotbase"))
          ...
          if ((*it1)[1].get_leaf() < (*it2)[1].get_leaf())
            return true;
          if ((*it1)[1].get_leaf() > (*it2)[1].get_leaf())
            return false;
        } else if (it1->isnode("slotindex") &&
                   ((*it1)[1].isnode("baseconst") ||
                    (*it1)[1].isnode("basevar")) &&
                   ((*it2)[1].isnode("baseconst") ||
                    (*it2)[1].isnode("basevar"))) {
          if ((*it1)[1][0].get_leaf() < (*it2)[1][0].get_leaf())
            return true;
          if ((*it1)[1][0].get_leaf() > (*it2)[1][0].get_leaf())
            return false;
        } else
    ...
    for (size_t i = 1; i + 1 < in.size(); i += 2) {
      ...
      if (tmp1.size() == tmp2.size()) {
        ...
        throw string("Rename of touples with unequal sizes is invalid.");
    }
    if (0 == (in.size() % 2)) {
      ...
      if (tmp1.size() == 1)
        ...
        throw string("Cannot prefix a tuple.");
    }

    return SimplifyIDLConstraint(
        in[0], lookup, indices, collidx,
        [renaming, &rename, &prefix](const SyntaxTree &var) {
          auto apply =
              renaming ? renaming : [](const SyntaxTree &x) { return x; };
          ...
          while (true) {
            ...
            if (it->isnode("slotbase")) {
              if (prefix.isnode())
                return apply(SyntaxTree("slotmember", {prefix, (*it)[0]}));
              else
                return apply(*it);
            } else if (it->isnode("slotmember"))
              apply = [old = apply,
                       member = (*it)[1]](const SyntaxTree &syntax) {
                return old(SyntaxTree("slotmember", {syntax, member}));
              };
            else if (it->isnode("slotindex"))
              apply = [old = apply,
                       index = (*it)[1]](const SyntaxTree &syntax) {
    ...
    return SyntaxTree("collect", {in[0], in[1],
                                  SimplifyIDLConstraint(in[2], lookup, indices,
                                                        collidx, renaming)});
  } else if (in.isnode("if")) {
    auto index1 = CollapseIDLIndex(in[0], indices);
    auto index2 = CollapseIDLIndex(in[1], indices);
    return SimplifyIDLConstraint(in[(index1 == index2) ? 2 : 3], lookup,
                                 indices, collidx, renaming);
    ...
    if (find_it == lookup.end())
      throw string("Included specification \"" + in[0].get_leaf() +
                   "\" does not exist.\n");
    return SimplifyIDLConstraint(find_it->second, lookup, new_indices, collidx,
                                 renaming);
  } else
    throw string("Constraint formula constraints invalid node type \"" +
                 in.get_type() + "\".");
}
...
SyntaxTree FlattenIDLJunctions(SyntaxTree in) {
  if (in.isnode("conjunction") || in.isnode("disjunction")) {
    ...
    return (elements.size() == 1) ? elements.front()
                                  : SyntaxTree(in.get_type(), elements);
  } else if (in.isnode("collect")) {
...
int CollapseIDLIndex(SyntaxTree in, const unordered_map<string, int> &vars) {
  auto recurs = [&in, &vars](size_t i) {
    return CollapseIDLIndex(in[i], vars);
  };
  auto as_int = [&in](size_t i) { return atoi(in[i].get_leaf().c_str()); };
  auto lookup = [&in, &vars](size_t i) {
    auto find_it = vars.find(in[i].get_leaf());
    if (find_it == vars.end())
      throw string("Undefined variable \"" + in[1].get_leaf() +
                   "\" in index calculations.");
    return find_it->second;
  };
  if (in.isnode("basevar") && in.size() == 1)
    return lookup(0);
  if (in.isnode("baseconst") && in.size() == 1)
    return as_int(0);
  if (in.isnode("addvar") && in.size() == 2)
    return recurs(0) + lookup(1);
  if (in.isnode("addconst") && in.size() == 2)
    return recurs(0) + as_int(1);
  if (in.isnode("subvar") && in.size() == 2)
    return recurs(0) - lookup(1);
  if (in.isnode("subconst") && in.size() == 2)
    return recurs(0) - as_int(1);
  throw string("Index calculation constraints invalid node type \"" +
               in.get_type() + "\".");
}
...
vector<SyntaxTree> NormaliseIDLVar(SyntaxTree in,
  ...
  if (in.isnode("slotbase"))
    ...
    bool iscollidx = in[1].isnode("basevar") && !collidx.empty() &&
                     collidx.back() == in[1][0].get_leaf();
    for (auto tmp : NormaliseIDLVar(
             in[0], indices,
             {collidx.begin(), collidx.end() - (iscollidx ? 1 : 0)}))
      result.push_back(
          {"slotindex",
           {tmp, iscollidx ? in[1]
                           : const_syntax(CollapseIDLIndex(in[1], indices))}});
    ...
    throw string("Variable slot constraints invalid node type \"" +
                 in.get_type() + "\".");
  return result;
}
